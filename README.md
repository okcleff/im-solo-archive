...
- **개별 상세 페이지**: `/season/[seasonNo]/[gender]/[handle]` 정적 생성
- **다크 모드**: 시스템 테마 연동 + 수동 토글 + 다크 모드를 디폴트로 하여 초기 플래시 방지
