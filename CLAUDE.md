...
- **커스텀 CSS 클래스** (`@layer components`에 정의):
  - `glass-panel` — 유리 효과 패널 (hero, archive status 섹션)
  - `surface-card` — 카드 기본 스타일 (ParticipantCard 등)
  - `section-title` — 소형 대문자 섹션 레이블
  - `soft-stat` — 통계 수치 박스
  - `detail-hero-shell`, `detail-hero-panel`, `detail-hero-photo` — 상세 페이지 hero
- **폰트 변수**: `var(--font-sans)`, `var(--font-title)`. 웹폰트는 CDN 로딩.
