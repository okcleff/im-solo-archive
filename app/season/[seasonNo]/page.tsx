...
import { getSiteUrl, getParticipantUrl, getParticipantSummary } from '@/lib/utils';
...
export default async function SeasonPage({ params }: Props) {
  ...
  const jsonLd = {
    ...
    description: `나는 SOLO ${season.label} 출연자 프로필 아카이브`,
    ...
    <>
      ...
      <section className="bg-gradient-to-br from-slate-900 to-slate-800 text-white py-10 px-4">
        <div className="max-w-6xl mx-auto">
          <Link
            href="/"
            className="text-slate-400 hover:text-white text-sm mb-3 inline-flex items-center gap-1 transition-colors"
          >
            ← 전체 기수
          </Link>
